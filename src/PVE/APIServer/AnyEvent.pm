...
sub finish_response {
    ...
    if (!$self->{end_loop} && $reqstate->{keep_alive} > 0) {
        ...
        $hdl->on_drain(sub {
            eval { $self->client_do_disconnect($reqstate); };
            warn $@ if $@;
        });
    }
}
...
sub websocket_proxy {
    ...
    eval {
        ...
        tcp_connect $remhost, $remport, sub {
            ...
            my $res =
                "$proto 101 Switching Protocols\015\012"
...
sub proxy_request {
    ...
    eval {
        ...
        $headers->{'cookie'} =
            PVE::APIServer::Formatter::create_auth_cookie($auth->{ticket}, $self->{cookie_name})
            if $auth->{ticket};
        $headers->{'Authorization'} = PVE::APIServer::Formatter::create_auth_header(
            $auth->{api_token},
            $self->{apitoken_name},
        ) if $auth->{api_token};
    ...
    eval {
        ...
        if (!defined($formatter)) {
            $self->error(
                ...
                "no formatter for uri $rel_uri, $format",
            );
            return;
        }
        ...
        my ($raw, $ct, $nocomp) =
            $formatter->($res, $res->{data}, $params, $path, $auth, $self->{formatter_config});
...
sub handle_spice_proxy_request {
    ...
    eval {
        ...
        tcp_connect $remhost, $remport, sub {
            ...
            if ($remip) {
                my $header =
                    "CONNECT ${connect_str} $proto\015\012"
                ...
                $reqstate->{proxyhdl}->push_read(
                    line => sub {
                        ...
                        if ($line =~ m!^$proto 200 OK$!) {
                            ...
                            $reqstate->{proxyhdl}->unshift_read(
                                line => sub {
                                    &$startproxy();
                                },
                            );
...
sub file_upload_multipart {
    ...
    eval {
        ...
        if ($rstate->{phase} == 2) { # Phase 2 - dump content into file
            ...
            if ($write_length > 0) {
                syswrite($rstate->{outfh}, $data) == $write_length
                    or die "write to temporary file failed - $!\n";
        ...
        if ($rstate->{phase} == 100) { # Phase 100 - transfer finished
            ...
            syslog(
                'info',
                "multipart upload complete (size: %dB time: %.3fs rate: %.2fMiB/s filename: %s)",
                $rstate->{bytes},
                $elapsed,
                $rstate->{bytes} / ($elapsed * 1024 * 1024),
    ...
    if ($self->{spiceproxy}) {
        ...
        if (!$ticket) {
            $api_token = PVE::APIServer::Formatter::extract_auth_value(
                $auth_header, $self->{apitoken_name},
            );
        }
                    ...
                    if ($line =~ /(\S+)\040(\S+)\040HTTP\/(\d+)\.(\d+)/o) {
                        ...
                        if ($maj != 1) {
                            $self->error(
                                $reqstate,
                                506,
                                "http protocol version $maj.$min not supported",
                            );
                            return;
                        }
                        if ($url =~ m|^[^/]*@|) {
    ...
    eval {
        ...
        while ($clientfh = $self->accept()) {
            ...
            if (my $sin = getpeername($clientfh)) {
                ...
                ($reqstate->{peer_port}, $reqstate->{peer_host}) =
                    ($pport, Socket::inet_ntop($pfamily, $phost));
            } else {
...
sub new {
    ...
    $self->{max_conn_soft_limit} =
        $self->{max_conn} > 100 ? $self->{max_conn} - 20 : $self->{max_conn};

    $self->{socket_watch} = AnyEvent->io(
        fh => $self->{socket},
        poll => 'r',
        cb => sub {
            eval {
    ...
    $self->{term_watch} = AnyEvent->signal(
        signal => "TERM",
        cb => sub {
    ...
    $self->{quit_watch} = AnyEvent->signal(
        signal => "QUIT",
        cb => sub {
    ...
    $self->{inotify_poll} = AnyEvent->timer(
        after => 5,
        interval => 5,
        cb => sub {
